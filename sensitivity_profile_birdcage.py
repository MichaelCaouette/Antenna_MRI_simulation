...
# The design to use
from jefimenko_acsolver.model_birdcage import BirdCage
...
R_cylinder = 0.040  # m, radius of the cyclinder

self = BirdCage(R=R_cylinder, 
                height=R_cylinder, 
                N_rung=8,
                N_smoothloop=25)
...
my_freq = 297.2*1e6 # Hz
my_delays = self.get_delays_optimal_rotation(my_freq)
my_freqs   = zeros + my_freq  # Same frequency in every element
Irung = 5 # Amp, the maximum current in one rung. The exact value doesn't matter
my_Is     =  self.get_current_optimal_rotation(Irung)
...
# At some point, this will all be taken from one script 
# It is the volume that is 1 where the coil are and 0 otherwise. 
epsilon = 0.01 # m, radius of the wire

from mask_segments import get_min_dist
...
for wire in my_loops:
    # Loop over each pair of points defining the wire
    n_pts = len(wire)
    for i in range(n_pts-1):
        # Compute the distance between this segment and the point probed
        p1 = wire[i]
        p2 = wire[i+1]
        dist = get_min_dist(p1, p2, 
                            [x_probe, y_probe, z_probe])
        
        # dist is a meshgrid of the distance between this particile point on the 
        # wire and all the point probed on the grid. 
        # Add the wire where they should be
        wires_map += np.where(dist<=epsilon, 1, 0) # One is d2<=eps, zero otherwise
