...
class Show3DVectorField():
    ...
    def add_list_wire(self, list_wire, list_color=[], 
        ...
        for ii, wire in enumerate(list_wire):
            ...
            if ii == 0:
                self.add_line(xline, yline, zline, color=my_color, 
                             label=label, show_corner=show_corner) 
            else:
                self.add_line(xline, yline, zline, color=my_color, 
                                 label='', show_corner=show_corner)     
            
        # Add a cool colorbar if we have enough color
    ...
    def add_bonding_box(self, 
                        xmin, xmax,
                        ymin, ymax,
                        zmin, zmax):
        """
        Add a box bonded by the input. 
        Display only the edges. 
        """
        
        # Describe All the 12 edges
        list_path = [[[xmin, xmax], [ymin, ymin], [zmin, zmin]], # In x direction
                     [[xmin, xmax], [ymax, ymax], [zmin, zmin]],
                     [[xmin, xmax], [ymin, ymin], [zmax, zmax]],
                     [[xmin, xmax], [ymax, ymax], [zmax, zmax]],
                     [[xmin, xmin], [ymin, ymax], [zmin, zmin]], # In y direction
                     [[xmax, xmax], [ymin, ymax], [zmin, zmin]],
                     [[xmin, xmin], [ymin, ymax], [zmax, zmax]],
                     [[xmax, xmax], [ymin, ymax], [zmax, zmax]],
                     [[xmin, xmin], [ymin, ymin], [zmin, zmax]], # In z direction
                     [[xmax, xmax], [ymin, ymin], [zmin, zmax]],
                     [[xmin, xmin], [ymax, ymax], [zmin, zmax]],
                     [[xmax, xmax], [ymax, ymax], [zmin, zmax]]]
        
        for my_path in list_path:
            self.ax.plot3D(my_path[0], my_path[1], my_path[2], '-', color='k')  
