...
class FieldLine():
    """
    Magnetic field from Jefimenko equation from a line (ie a list of segment)
    
    This is the true time dependant field from E&M solution.
    We split the line into linear segment and add the solution from these
    line segments. 
    ...
    def __init__(self, list_vec,
        ...
        # defne the radial frequency once and for all
        self.w = 2*np.pi*f
    ...
    def get_field_from_spatial(self, vcos, vsin, t):
        ...
        components. 
        """
        _debug('FieldLine.get_field_from_spatial')
    ...
    def get_field_spatial(self, x, y, z, print_progress=False):
        ...
        """
        _debug('FieldLine.get_field_spatial')
        ...
        for i in range(self.N_segment):
            if print_progress:
                print('FieldLine.get_field_spatial --> Calculating %d/%d'%(i, self.N_segment))
            
            # Create the line segement
            self.vec_r1 = self.list_vec[i]
            self.vec_r2 = self.list_vec[i+1]
            self.field_segment = FieldSegment(self.vec_r1, 
                                                  self.vec_r2, 
                                                  I=self.I, 
                                                  f=self.f,
                                                  u0 = self.u0,
...
if __name__ == '__main__':
    ...
    self = FieldLine(list_pts_loop, I=I, f=f)
